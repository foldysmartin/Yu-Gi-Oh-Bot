...
def test_can_summon_a_monster():
    ...
    game_state, abstract_field = monster.activate().apply(GameState(), abstract_field)
...
def test_cannot_summon_if_zone_is_not_empty():
    ...
    # Fill all 5 zones
    for i in range(1, 6):
        game_state, abstract_field = monster.activate().apply(
            GameState(), abstract_field
        )

    with raises(SummoningError):
        monster.activate().apply(GameState(), abstract_field)
...
def test_can_only_normal_summon_once_per_turn():
    ...
    game_state, abstract_field = monster.activate().apply(game_state, abstract_field)
